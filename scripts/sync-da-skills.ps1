...
function Invoke-Mirror {
  ...
  $args = @($Source, $Destination, '/MIR', '/NFL', '/NDL', '/NJH', '/NJS', '/NP', '/R:1', '/W:1')
  if ($excludeAll.Count -gt 0) { $args += '/XD'; $args += $excludeAll }
  if ($DryRun) { $args += '/L' }

  & robocopy @args | Out-Null
  # robocopy exit codes: 0 = nothing, 1 = copied, 2 = extras, 3 = copied+extras, >=8 = error
...
if ($diffs.Count -eq 0) {
  Write-Host "No drift. Both sides are identical (" -NoNewline -ForegroundColor Green
  Write-Host "$($snapA.Count) files" -NoNewline
...
Write-Host "Drift detected - $($diffs.Count) file(s):" -ForegroundColor Yellow
...
switch ($Mode) {
  ...
  'push' {
    ...
    Invoke-Mirror -Source $workspaceSkills -Destination $projectsSkills -DryRun:$DryRun | Out-Null
    if ($DryRun) {
  ...
  'pull' {
    ...
    Invoke-Mirror -Source $projectsSkills -Destination $workspaceSkills -DryRun:$DryRun | Out-Null
    if ($DryRun) {
      Write-Host "DRY RUN complete - no changes written." -ForegroundColor Yellow
    } else {
